...
class JobTrackerApp:
    def __init__(self, root):
        ...
        self.scrape_button = ttk.Button(root, text="🔍 Run Scraper Now", command=self.run_scraper, bootstyle=PRIMARY)
    ...
    def run_scraper(self):
        self.scrape_button.config(text="Scraping...", state=DISABLED)
        self.root.update()
        jobs = scrape_jobs()
        if jobs:
            df = pd.DataFrame(jobs)
            df.to_csv(JOBS_FILE, index=False)
            messagebox.showinfo("Success", f"{len(jobs)} jobs found and saved to jobs.csv")
        else:
            messagebox.showinfo("No Matches", "No matching jobs found.")
        self.scrape_button.config(text="🔍 Run Scraper Now", state=NORMAL)
